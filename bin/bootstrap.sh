...
#   COWORKER_REF          版本（默认取远端最新 tag；解析失败回退脚本内兜底值）
...
if [ -z "$COWORKER_REF" ]; then
  ...
  if [ -n "$LATEST_TAG" ]; then
    COWORKER_REF="@${LATEST_TAG}"
  else
    COWORKER_REF="$COWORKER_REF_FALLBACK"
    warn "未能从远端解析最新 tag，回退到 ${COWORKER_REF_FALLBACK}"
  fi
fi
