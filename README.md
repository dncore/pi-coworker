...
| 集群 | 工具 | 命令 |
|---|---|---|
| onboarding 入职引导 | `coworker_check_env` / `coworker_config_init` / `coworker_auth_login` / `coworker_auth_complete` / `coworker_auth_status` / `coworker_bot_setup` | `/coworker:setup` `/coworker:status` `/coworker:bot` |
